...
var sendMessageByGroup = function (channelService, route, msg, groups, opts, cb) {
  ...
  logger.debug('channelService sendMessageByGroup', { serverId: app.serverId, route, msg, groups, opts });
  if (count === 0) {
    // group is empty
    utils.invokeCallback(cb);
    return;
  ...
  var rpcCB = function (serverId) {
    return function (err, fails) {
      if (err) {
        logger.error('to dispatch msg', { serverId, stack: err.stack });
        latch.done();
        return;
      }
      if (fails) {
...
var addToStore = function (self, key, value) {
  if (!!self.store) {
    self.store.add(key, value, function (err) {
      if (!!err) {
        logger.error('add key with err', { key, value, stack: err.stack });
      }
    });
  }
};
...
var removeFromStore = function (self, key, value) {
  if (!!self.store) {
    self.store.remove(key, value, function (err) {
      if (!!err) {
        logger.error('remove key with err', { key, value, stack: err.stack });
      }
    });
  }
};
...
var loadAllFromStore = function (self, key, cb) {
  if (!!self.store) {
    self.store.load(key, function (err, list) {
      if (!!err) {
        logger.error('load key with err', { key, stack: err.stack });
        utils.invokeCallback(cb, err);
      } else {
...
var removeAllFromStore = function (self, key) {
  if (!!self.store) {
    self.store.removeAll(key, function (err) {
      if (!!err) {
        logger.error('remove key with err', { key, stack: err.stack });
      }
    });
  }
};
