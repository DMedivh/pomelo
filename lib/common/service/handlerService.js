...
var forwardLogger = require('pomelo-logger-ts').getLogger('forward-log', __filename);
/**
 * Handler service.
...
Service.prototype.handle = function (routeRecord, msg, session, cb) {
  ...
  if (!handler) {
    logger.error('fail to find handler for', { msg });
  ...
  if (!Array.isArray(msg)) {
    var promise = handler[method](msg, session, callback);
    if (promise.catch) {
      promise.catch((err) => {
        callback(null, { code: 505 });
      });
    }
  } else {
    msg.push(session);
...
Service.prototype.getHandler = function (routeRecord) {
  ...
  if (typeof handler[routeRecord.method] !== 'function') {
    logger.warn('could not find the method', { routeRecord });
    return null;
  }
  return handler;
};
