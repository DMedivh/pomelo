...
Remote.prototype.forwardMessage = function (msg, session, cb) {
  ...
  if (!server) {
    logger.error('server component not enable on', { serverId: this.app.serverId });
    utils.invokeCallback(cb, new Error('server component not enable'));
    return;
  }

  if (!sessionService) {
    logger.error('backend session component not enable on', { serverId: this.app.serverId });
  ...
  logger.debug('handle message', { serverId: this.app.serverId, msg });
  ...
  server.handle(msg, backendSession, function (err, resp, opts) {
    ...
    logger.error('failed', { serverId: this.app.serverId, msg, message: err.message || err.stack });
    utils.invokeCallback(cb, null, { code: 500 }, opts);
  });
};
