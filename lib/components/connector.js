var logger = require('pomelo-logger-ts').getLogger('pomelo', __filename);
...
pro.send = function (reqId, route, msg, recvs, opts, cb) {
  logger.debug('send message', { serverId: this.app.serverId, reqId, route, msg, recvs, opts });
...
var hostFilter = function (cb, socket) {
  ...
  if (!!this.blacklistFun && typeof this.blacklistFun === 'function') {
    ...
    self.blacklistFun(function (err, list) {
      if (!!err) {
        logger.error('connector blacklist error', { stack: err.stack, message: err.message });
        utils.invokeCallback(cb, self, socket);
        return;
      }
      if (!Array.isArray(list)) {
        logger.error('connector blacklist is not array', { list });
        utils.invokeCallback(cb, self, socket);
        return;
      }
      if (!!check(list)) {
...
var bindEvents = function (self, socket) {
  ...
  if (self.connection && maxConnections) {
    ...
    if (statisticInfo.totalConnCount > maxConnections) {
      logger.warn('the server has reached the max connections', { id: curServer.id, maxConnections });
      socket.disconnect();
      return;
    }
  }
...
var handleMessageAsync = function (self, msg, session, socket) {
  if (self.decode) {
    self.decode(msg, session, function (err, dmsg) {
      if (err) {
        logger.error('fail to decode message from client .', { stack: err.stack });
        return;
      }
...
var getSession = function (self, socket) {
  ...
  logger.debug('getSession session is created with session id', { getServerId: app.getServerId(), sid });
  ...
  session.on('bind', function (uid) {
    logger.debug('session bind with uid', { serverId: self.app.serverId, uid, id: sid });
...
var handleMessage = function (self, session, msg) {
  logger.debug('handleMessage', { serverId: self.app.serverId, id: session.id, msg });
  ...
  self.server.globalHandle(msg, session.toFrontendSession(), function (err, resp, opts) {
    if (resp && !msg.id) {
      logger.warn('try to response to a notify', { route: msg.route, response: resp });
      return;
    }
...
var verifyMessage = function (self, session, msg) {
  ...
  if (!sig) {
    logger.error('receive data from client has no signature', { serverId: self.app.serverId });
    return false;
  }
  ...
  if (!session.get('pubKey')) {
    ...
    if (!!pubKey) {
      ...
      logger.error('could not get public key, session id', { id: session.id });
      return false;
    }
  } else {
  ...
  if (!pubKey.n || !pubKey.e) {
    logger.error('could not verify message without public key', { serverId: self.app.serverId });
    return false;
  }
