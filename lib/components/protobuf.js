...
var protobuf = require('pomelo-protobuf-ts');
...
pro.onUpdate = function (type, path, event) {
  ...
  fs.readFile(path, 'utf8', function (err, data) {
    try {
      ...
      logger.info('proto changed', { type, path, version: self.version });
    } catch (e) {
      logger.warn("proto change failed", { path, message: e.message, stack: e.stack });
    }
  });
};
