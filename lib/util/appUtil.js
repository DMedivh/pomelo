...
module.exports.optComponents = function (comps, method, cb) {
  ...
  async.forEachSeries(comps, function (comp, done) {
    ...
    if (err) {
      logger.error('fail to operate component, method', { method, message: err.message || err.stack || err });
    }
...
var loadServers = function (app) {
  ...
  for (var serverType in servers) {
    ...
    for (i = 0, l = slist.length; i < l; i++) {
      ...
      if (server.wsPort) {
        logger.warn('wsPort is deprecated, use clientPort in frontend server instead, server', { server });
      }
    }
  }
...
var loadLifecycle = function (app) {
  ...
  for (var key in lifecycle) {
    if (typeof lifecycle[key] === 'function') {
      ...
      logger.warn('lifecycle.js in is error format.', { filePath });
    }
  }
};
