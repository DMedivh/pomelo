...
Connector.prototype.start = function (cb) {
  ...
  this.socket = dgram.createSocket(this.type, function (msg, peer) {
    var key = genKey(peer);
    if (!self.clients[key]) {
      var udpsocket = new UdpSocket(new ObjectID().toHexString(), self.socket, peer);
  ...
  this.socket.on('error', function (err) {
    logger.error('udp socket encounters with error', { message: err.message, stack: err.stack });
    return;
  });
