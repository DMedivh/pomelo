...
Watchdog.prototype.record = function (id) {
  if (!this.isStarted && --this.count < 0) {
    ...
    logger.info('all servers startup', { usedTime });
...
Watchdog.prototype.notifyById = function (id, msg) {
  this.service.agent.request(id, Constants.KEYWORDS.MONITOR_WATCHER, msg, function (signal) {
    if (signal !== Constants.SIGNAL.OK) {
      logger.error('master watchdog fail to notify to monitor', { id, msg });
    } else {
      logger.debug('master watchdog notify to monitor success', { id, msg });
    }
  });
};
...
Watchdog.prototype.notify = function (msg) {
  ...
  if (count === 0) {
    logger.warn('master watchdog listeners is none, msg', msg);
    return;
  }
  var latch = countDownLatch.createCountDownLatch(count, { timeout: Constants.TIME.TIME_WAIT_COUNTDOWN }, function (isTimeout) {
    if (!!isTimeout) {
      ...
      logger.error('master watchdog request timeout message', { msg, timeouts, fails });
    }
    if (!success) {
      logger.error('master watchdog request fail message', { msg, fails });
    }
  });
