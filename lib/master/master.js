...
var crashLogger = require('pomelo-logger-ts').getLogger('crash-log', __filename);
var adminLogger = require('pomelo-logger-ts').getLogger('admin-log', __filename);
...
Server.prototype.start = function (cb) {
  ...
  this.masterConsole.on('error', function (err) {
    if (!!err) {
      logger.error('masterConsole encounters with error', { stack: err.stack, message: err.message });
      return;
    }
  });
  ...
  this.masterConsole.on('disconnect', function (id, type, info, reason) {
    crashLogger.info('disconnect', { type, id, timestamp: Date.now(), reason: reason || 'disconnect' });
    var count = 0;
    var time = 0;
    var pingTimer = null;
  ...
  this.masterConsole.on('admin-log', function (log, error) {
    if (error) {
      adminLogger.error('failed', { log });
    } else {
      adminLogger.info('info', { log });
    }
  });
};
