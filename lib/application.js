...
Application.init = function (opts) {
  ...
  logger.info('application inited', this.getCurServer());
};
 ...
 * 
 * @param {Object} logger pomelo-logger-ts instance without configuration
 *
 * @memberOf Application
 */
...
Application.load = function (name, component, opts) {
  ...
  if (name && this.components[name]) {
    ...
    logger.warn('ignore duplicate component', { name });
    return;
  }
...
Application.loadConfigBaseApp = function (key, val, reload) {
  ...
  if (fs.existsSync(originPath)) {
    ...
    logger.error('invalid configuration with file path', { key });
  }
    ...
    var startUp = function () {
      appUtil.optComponents(self.loaded, Constants.RESERVED.START, function (err) {
        ...
        if (err) {
          ...
          logger.info('enter after start...', self.getCurServer());
          self.afterStart(cb);
        }
      });
    };
...
Application.afterStart = function (cb) {
  ...
  appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function (err) {
    ...
    if (!err) {
      logger.info('finish start', self.getCurServer());
    }
    if (!!afterFun) {
    ...
    logger.info('startup', { id, usedTime });
...
Application.stop = function (force) {
  if (this.state > STATE_STARTED) {
    logger.warn('application is not running now.');
    return;
  }
...
Application.use = function (plugin, opts) {
  ...
  fs.readdirSync(plugin.components).forEach(function (filename) {
    ...
    if (!fs.existsSync(absolutePath)) {
      logger.error('component  not exist', { name, absolutePath });
    } else {
  ...
  if (!plugin.events) {
    ...
    if (!fs.existsSync(plugin.events)) {
      logger.error('fail to find events, find path', { events: plugin.events });
      return;
    }
    ...
    fs.readdirSync(plugin.events).forEach(function (filename) {
      ...
      if (!fs.existsSync(absolutePath)) {
        logger.error('events not exist', { filename, absolutePath });
      } else {
