...
Module.prototype.monitorHandler = function (agent, msg, cb) {
  ...
  if (!func) {
    logger.info('monitorwatcher unknown action', { action: msg.action });
    return;
  }
...
var subscribeRequest = function (self, agent, id, cb) {
  ...
  agent.request(Constants.KEYWORDS.MASTER_WATCHER, msg, function (err, servers) {
    if (err) {
      logger.error('subscribeRequest request to master with error', { stack: err.stack });
...
var addServer = function (self, agent, msg, cb) {
  logger.debug('receive addServer signal', { serverId: self.app.serverId, msg });
  if (!msg || !msg.server) {
    logger.warn('monitorwatcher addServer receive empty message', msg);
...
var removeServer = function (self, agent, msg, cb) {
  logger.debug('receive removeServer signal', self.app.serverId, msg);
  if (!msg || !msg.id) {
    logger.warn('monitorwatcher removeServer receive empty message', msg);
    utils.invokeCallback(cb, Constants.SIGNAL.FAIL);
    return;
  }
...
var replaceServer = function (self, agent, msg, cb) {
  logger.debug('receive replaceServer signal', { serverId: self.app.serverId, msg });
  if (!msg || !msg.servers) {
    logger.warn('monitorwatcher replaceServer receive empty message', msg);
    utils.invokeCallback(cb, Constants.SIGNAL.FAIL);
    return;
  }
